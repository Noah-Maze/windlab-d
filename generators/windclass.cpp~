...
windclass::windclass(MODULE *module)
{
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_double, "mv_avg_wind_speed", PADDR(mv_avg_wind_speed),
...
double windclass::agg_lin_pow(double wind){
 ...
 double total = 0;
 total = 	 ( ( 0.0214 * ( lin_pow(wind + (-3*spatialStd) ) + lin_pow(wind + (-2*spatialStd) ) ) ) + 
...
double windclass::agg_log_pow(double wind){
 double total = 0;
 total =         ( ( 0.0214 * ( log_pow(wind + (-3*spatialStd) ) + log_pow(wind + (-2*spatialStd) ) ) ) + 
